...
<p>このリポジトリは、sedコマンドを実行して与えられた文字列から指定した文字で1回だけ置換する場合と複数置換する場合を体験できます</p>
</div>
...
<div class="wp-block-group">
<p>上記のコマンドをインストール済みの方は、以下のコマンドを実行してリポジトリからダウンロード後、ディレクトリを移動し、sedコマンドを実行して置換された文字列を出力してみてください</p>
...
<div class="wp-block-group">
<pre class="wp-block-own-copy-code-line-block wp-block-code has-24292-eff-color has-cyan-bluish-gray-background-color has-text-color has-background has-1-125-rem-font-size"><code>echo "hello sed command world" | sed 's/e/E/'
echo "hello sed command world" | sed 's/e/E/g'</code></pre>



<details class="wp-block-details"><summary>出力結果</summary>
<pre class="wp-block-code has-background" style="background-color:#ffeeee"><code>hEllo sed command world
hEllo sEd command world</code></pre>
</details>
</div>
...
<pre class="wp-block-code has-24292-eff-color has-text-color has-background has-1-125-rem-font-size" style="background-color:#ffeeee"><code>hEllo sed command world
hEllo sEd command world</code></pre>
</details>
</div>
</details>
...
<div class="wp-block-group">
<p>'s/e/E/'は行で一番初めに合致したeをEに置換するだけですが、's/e/E/g'はgのglobalオプションをつけることで行に出現するeを全てEに置換します<br>以下のコードを実行して各行の文字列が置換されていることを確認してみてください</p>



<pre class="wp-block-own-copy-code-line-block wp-block-code has-24292-eff-color has-cyan-bluish-gray-background-color has-text-color has-background has-1-125-rem-font-size"><code>echo -e "apple\nbanana\ncherry" | sed 's/a/A/'
echo -e "apple\nbanana\ncherry" | sed 's/a/A/g'</code></pre>
</div>
